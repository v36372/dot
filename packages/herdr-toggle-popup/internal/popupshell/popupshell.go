...
const (
	...
	// Delay before enabling the in-popup C-n hide bind so the keystroke that opened
...
# Install hide bind after a short delay so the opening ctrl+n cannot race-close us.
(
  sleep 0.4
  "$tmux" -L herdr-toggle-popup -f "$conf" bind-key -n C-n run-shell -b "$hide_cmd" 2>/dev/null || true
) >/dev/null 2>&1 &
